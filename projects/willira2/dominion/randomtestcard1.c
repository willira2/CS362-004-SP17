...
int main(){
	int testHolder = 1, testScore = 1, smitCount1 = 0, smitCount2 = 0;
	int i, j, m, handPos;
	...
	for(i = 0; i < TEST_NUMBER; i++){
		...
		//play smithy for each player in random game
		for(j = 0; j < masterG.numPlayers; j++){
		    ...
		    //put smithy card at handpos
		    controlG.hand[j][handPos] = smithy;

		  	for(m = 0; m < controlG.handCount[j]; m++){
				if (controlG.hand[j][m] == smithy){
					smitCount1++;
				}
		    }
			...
			//play smithy card
			playSmithy(j, &testG, handPos);

		    for(m = 0; m < controlG.handCount[m]-1; m++){
		    	if (testG.hand[m][i] == smithy){
		    		smitCount2++;
		    	}
		    }

			//compare control and test games
			if(!assertTrue(testG.handCount[j], controlG.handCount[j]+2)){
		    	printf("\tTEST FAILURE: hand size: %d\texpected: %d\n", testG.handCount[j], controlG.handCount[j]+2);
		    	testHolder=0;
		    }
		    
		    if(!assertTrue(smitCount1-1, smitCount2)){
		    	printf("\tTEST FAILURE: smithy cards in hand: %d\texpected: %d\n", smitCount2, smitCount2-1);
		    	testHolder=0;
		    }
