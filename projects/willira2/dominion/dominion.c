...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case sea_hag:
      ...
      /*for (i = 0; i < state->numPlayers+1; i++){ //INCORRECT - SHOULD BE i < state->numPlayers. THIS IS A BUG
	if (i != currentPlayer){
...
/*int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case feast: //PASS int currentPlayer, struct* gameState state int temphand[], int choice1
	     return playFeast(currentPlayer, state, temphand, handPos);
			
    case gardens:
      	return playGardens();
			
    case mine: //PASS int currentPlayer, struct* gameState state, int choice1, int choice2, int handPos
    	return playMine(currentPlayer, state, choice1, choice2, handPos);
			
    case remodel: // PASS int currentPlayer, struct* gameState state, int choice1, int choice2, int handPos	 
    	return playRemodel(currentPlayer, state, choice1, choice2, handPos);  

    case smithy: // PASS int currentPlayer, struct* gameState state, int handPos
    	return playSmithy(currentPlayer, state, handPos);
		
    case village: // PASS int currentPlayer, struct gameState *state, int handPos
    	return playVillage(currentPlayer, state, handPos);
		
    case baron: // int currentPlayer, struct gameState *state, int choice1
    	return playBaron(currentPlayer, state, choice1);
		
    case great_hall:// int currentPlayer, struct gameState *state, int handPos 
	    return playGreatHall(currentPlayer, state, handPos);
		
    case minion: // int currentPlayer, struct gameState *state, int handPos, int choice1, int choice2
    	return playMinion(currentPlayer, state, handPos, choice1, choice2);
			
    case steward: // int currentPlayer, struct gameState *state, int handPos, int choice1, int choice2, int choice3
    	return playSteward(currentPlayer, state, handPos, choice1, choice2, choice3);
		
    case tribute: // int currentPlayer, struct gameState *state, int nextPlayer
    	return playTribute(currentPlayer, state, nextPlayer);
		
    case ambassador: // int currentPlayer, struct gameState *state, int choice1, int choice2, int handPos
	    return playAmbassador(currentPlayer, state, choice1, choice2, handPos);
		
    case cutpurse: // int currentPlayer, struct gameState *state, int handPos
    	return playCutpurse(currentPlayer, state, handPos);
		
    case embargo: // int currentPlayer, struct gameState *state, int handPos, int choice1
    	return playEmbargo(currentPlayer, state, handPos, choice1);
		
    case outpost: // int currentPlayer, struct gameState *state, int handPos
    	return playOutpost(currentPlayer, state, handPos);

    case salvager: // int currentPlayer, struct gameState *state, int handPos, int choice1
    	return playSalvager(currentPlayer, state, handPos, choice1);
		
    case sea_hag: // int currentPlayer, struct gameState *state
    	return playSeaHag(currentPlayer, state);
		
    case treasure_map: // int currentPlayer, struct gameState *state, int handPos
    	return playTreasureMap(currentPlayer, state, handPos);
...
int playSmithy(int currentPlayer, struct gameState *state, int handPos)
{
 ...
 int playVillage(int currentPlayer, struct gameState *state, int handPos)
 {
      ...
      //+2 Actions
      //state->numActions = state->numActions + 2;
      state->numActions = 2; //	INCORRECT - THIS IS A BUG. This resets numActions to 2 rather than adding 2 to it
