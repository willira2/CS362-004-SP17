...
int main(){
  int testHolder = 1, testScore = 1;
  int i, j, m, r, handPos, expectedHC, expectedC, expectedPC, controlDC;
  ...
  for(i = 0; i < TEST_NUMBER; i++){
    ...
    //play steward for each player in random game
    for(j = 0; j < masterG.numPlayers; j++){
        ...
        controlDC = controlG.deckCount[j];

        //fill out hand
        for (m = 0; m < 5; m++){
          drawCard(j, &controlG);
        }
        printf("hand count of control: %d\n", controlG.handCount[j]);
        
        updateCoins(controlG.whoseTurn, &controlG, 0);

        //set handPos to random value
        handPos = rand() % controlG.handCount[j];

        //put steward card at handPos
        controlG.hand[j][handPos] = steward;

        choice1 = rand() % 3;
        printf("choice 1: %d\n", choice1 );

        if((choice1 != 1) && (choice1 != 2)){
          if(controlG.handCount[j]>2){
            do{
              r = rand() % controlG.handCount[j];
              choice2 = r;
            }while(r == handPos);
             ...
             do{
              r = rand() % controlG.handCount[j];
              choice3 = r;
            }while((r == handPos) || (r == choice2));
          }

          else if(controlG.handCount[j]==2){
            do{
              r = rand() % controlG.handCount[j];
              choice2 = r;
            }while(r == handPos);
        ...
        //play steward card
        cardEffect(steward, choice1, choice2, choice3, &testG, handPos, NULL);

        if(choice1 == 1){
          expectedHC = controlG.handCount[j]+1;
          expectedC = controlG.coins;
          expectedPC = controlG.playedCardCount+1;
        }
        else if (choice1 == 2){
          expectedHC = controlG.handCount[j]-1;
          expectedC = controlG.coins+2;
          expectedPC = controlG.playedCardCount+1;
        }
        else{
          if(controlG.handCount[j] > 2){
            expectedHC = controlG.handCount[j]-3;
          }
          else if (controlG.handCount[j] < 2){
            expectedHC = 0;
          }
       ...
       //compare control and test games
        ...
        if (controlDC < 7){
          if(!(testG.handCount[j] <= controlG.handCount[j])){
          printf("\tTEST FAILURE: hand size");
          testHolder=0;
        }
      }

        else{
          if(!assertTrue(testG.handCount[j], expectedHC)){
            printf("\tTEST FAILURE: hand size: %d\texpected: %d\n", testG.handCount[j], expectedHC);
            testHolder=0;
          }
          }
            
      if(!assertTrue(testG.playedCardCount, expectedPC)){
          printf("\tTEST FAILURE: played card count: %d\texpected: %d\n", testG.discardCount[j], expectedPC);
          testHolder=0;
        }

      if(!assertTrue(testG.coins, expectedC)){
          printf("\tTEST FAILURE: coins: %d\texpected: %d\n", testG.handCount[j], expectedC);
          testHolder=0;
        }
  ...
  if(testScore){printf("steward card: PASS\n");}
      else{printf("steward card: FAIL\n");}  
